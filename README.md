...
`RwLock<>` dibutuhkan karena data notifikasi ini dipakai bersama oleh banyak request handler, sehingga akses concurrent harus disinkronkan agar tidak terjadi data race. Pada receiver, operasi read list_all_as_string bisa jauh lebih sering daripada operasi write. RwLock lebih cocok untuk pola ini karena mengizinkan banyak reader berjalan bersamaan selama tidak ada writer. Jika memakai `Mutex`, semua akses read dan write akan saling mengunci satu sama lain, sehingga concurrency menjadi tidak berguna meskipun workload didominasi operasi baca.
